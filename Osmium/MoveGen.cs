		...
		*/
			...
			out Piece attackerPiece)
		{
			...
			while (rookHits != 0)
			{
				...
				atkerSq = sq;
				attackerPiece = FindEnemyPiece(board, sq);
				if (++attackerCount > 1) return true;
			}
		...
		public static bool IsEscapePossible(Board board, bool isWhiteToMove)
		{
			...
			{
				byte targetSquare = (byte)BitOperations.TrailingZeroCount(possibleEscapes);
		...
		public static byte GetKingSquare(Board board, bool isWhite)
		{
			ulong kingBB = isWhite
				? board.bitboards[(int)Piece.WhiteKing]
				: board.bitboards[(int)Piece.BlackKing];

			return (byte)BitOperations.TrailingZeroCount(kingBB);
		}
		...
		{
			byte kingIdx = GetKingSquare(board, isWhiteDefend);
			return IsSquareAttacked(board, kingIdx, isWhiteDefend);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static Piece FindCapturedPiece(Board board, Square sq)
		{

			ulong mask = 1UL << (int)sq;
			int offset = ((int)board.sideToMove^1) * 6;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static Piece FindEnemyPiece(Board board, int sq)
		{
			ulong mask = 1UL << sq;
			int offset = ((int)board.sideToMove ^ 1) * 6;
